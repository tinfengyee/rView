<template>
  <view class="r-search" :style="{
    ...
    <view class="r-content" :style="{
      ...
      <view class="r-icon-wrap">
        <r-icon class="r-clear-icon" :size="30" :name="searchIcon" :color="searchIconColor ? searchIconColor : color"></r-icon>
      </view>
      <input confirm-type="search" @blur="blur" :value="value" @confirm="search" @input="inputChange" :disabled="disabled"
        @focus="getFocus" :maxlength="getMaxlength" :focus="focus" placeholder-class="r-placeholder-class" :placeholder="placeholder"
        :placeholder-style="`color: ${placeholderColor}`" class="r-input" type="text" :style="[{
					textAlign: inputAlign,
					color: color,
					backgroundColor: bgColor,
				}, inputStyle]" />
      <view class="r-close-wrap" v-if="keyword && clearabled && focused" @touchstart="clear">
        <r-icon class="r-clear-icon" name="close-circle-fill" size="34" color="#c0c4cc"></r-icon>
      </view>
    </view>
    <view :style="[actionStyle]" class="r-action" :class="[showActionBtn || show ? 'r-action-active' : '']"
      @touchstart.stop.prevent="custom">{{ actionText }}</view>
  </view>
</template>
...
<script>
   ...
   * @example <r-search placeholder="日照香炉生紫烟" v-model="keyword"></r-search>
   */
  export default {
    name: "r-search",
    props: {
