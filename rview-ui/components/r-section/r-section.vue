<template>
  <view class="r-section">
    <slot name="title">
      <view class="r-section-title" :style="{
        ...
        :class="{
			'r-section--line': showLine
		}">
        {{title}}
      </view>
    </slot>
    <view class="r-right-info" v-if="right" :style="{
			color: subColor
		}" @tap="rightClick">
      <slot name="right">
        {{subTitle}}
        <view class="r-icon-arrow">
          <r-icon name="arrow-right" size="24" :color="subColor"></r-icon>
        </view>
      </slot>
    </view>
  </view>
...
<script>
   ...
   * @example <r-section title="今日热门" :right="false"></r-section>
   */
  export default {
    name: "r-section",
    props: {
